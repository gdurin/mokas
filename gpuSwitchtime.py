...
def get_gpuSwitchTimeSkyrmions(stackImages, convolSize=10, 
    current_dev=None, ctx=None, block_size=(256,1), threshold=30, verbose=False):
    """
    Return a 3D-array with the positions of a step in a sequence for each pixel
    ...
    func_findswitches(convolStack_gpu, switch_gpu, dim_X, dim_Y, dim_Z, threshold32,  block=(block_X, block_Y, 1),
          grid=(grid_X2, grid_Y2))
    if verbose:
        print("Done")
        #Device to host copy  
...
if __name__ == "__main__":
    ...
    switches = get_gpuSwitchTimeSkyrmions(a, 3, current_dev=current_dev, ctx=ctx, block_size=(5,5), threshold=100)
